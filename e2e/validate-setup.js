...
const testFiles = fs
  .readdirSync(testDir)
  .filter((file) => file.endsWith(".js") && file !== "validate-setup.js");
...
testIds.forEach((testIdObj) => {
  ...
  const displayName =
    testIdObj.type === "dynamic" ? `${testIdObj.id} (dynamic)` : testIdObj.id;
  if (found) {
...
const e2eScripts = ["test:e2e", "test:e2e:headed", "test:e2e:ui"];
