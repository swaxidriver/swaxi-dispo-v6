...
test.describe("Swaxi Dispo v6 Smoke Tests", () => {
  ...
  test("should open template creation form in Administration", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('[data-testid="shift-template-manager"]'),
    ).toBeVisible();

    // Check that the template creation form is present
    await expect(
      page.locator('[data-testid="create-template-form"]'),
    ).toBeVisible();
    ...
    await expect(
      page.locator('[data-testid="template-start-time-input"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="template-end-time-input"]'),
    ).toBeVisible();
    await expect(
  ...
  test("should be able to create a template and generate shifts", async ({
    page,
  }) => {
  ...
  test("should display conflict badges when conflicts exist", async ({
    page,
  }) => {
    ...
    if (conflictCount > 0) {
      ...
      await expect(
        page.locator('[data-testid="conflict-tooltip"]'),
  ...
  test("should show version banner if update is available", async ({
    page,
  }) => {
