...
export function computeShiftConflicts(target, others, applications) {
  ...
  const overlapping = others.filter((o) =>
    overlaps(target.start, target.end, o.start, o.end),
  );
  ...
  if (target.assignedTo) {
    if (
      overlapping.some(
        (o) => o.assignedTo && o.assignedTo === target.assignedTo,
      )
    ) {
  ...
  if (target.assignedTo && target.workLocation) {
    if (
      overlapping.some(
        (o) =>
          o.assignedTo &&
          o.assignedTo === target.assignedTo &&
          o.workLocation &&
          o.workLocation !== target.workLocation,
      )
    ) {
