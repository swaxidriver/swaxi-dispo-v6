...
export function FeedbackProvider({ children, onNewFeedback }) {
  ...
  useEffect(() => {
    try { localStorage.setItem('feedback_items', JSON.stringify(items)) } catch { /* ignore */ }
  }, [items])

  const open = useCallback(() => setIsOpen(true), [])
  const close = useCallback(() => setIsOpen(false), [])

  const addFeedback = useCallback((message, { category = 'general', user } = {}) => {
    ...
    try { onNewFeedback?.(entry) } catch { /* ignore */ }
  ...
  const value = useMemo(() => ({ items, isOpen, open, close, addFeedback, exportJson }), [items, isOpen, open, close, addFeedback, exportJson])
  return (
