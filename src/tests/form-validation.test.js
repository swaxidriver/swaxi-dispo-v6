import { toast, validationRules, createValidator, formPersistence, createFormSubmitHandler } from '../ui/form-validation'
...
describe('Form Validation Utilities', () => {
  ...
  describe('validationRules', () => {
    test('required rule validates correctly', () => {
      ...
      expect(validationRules.required('', 'Name')).toBe('Name ist erforderlich')
    })
    ...
    test('email rule validates correctly', () => {
      ...
      expect(validationRules.email('invalid-email')).toBe('Bitte geben Sie eine gültige E-Mail-Adresse ein')
      expect(validationRules.email('invalid@')).toBe('Bitte geben Sie eine gültige E-Mail-Adresse ein')
    })
    ...
    test('time rule validates correctly', () => {
      ...
      expect(validationRules.time('25:00')).toBe('Bitte geben Sie eine gültige Uhrzeit ein (HH:MM)')
      expect(validationRules.time('14:60')).toBe('Bitte geben Sie eine gültige Uhrzeit ein (HH:MM)')
      expect(validationRules.time('invalid')).toBe('Bitte geben Sie eine gültige Uhrzeit ein (HH:MM)')
    })
    ...
    test('date rule validates correctly', () => {
      ...
      expect(validationRules.date('invalid-date')).toBe('Bitte geben Sie ein gültiges Datum ein')
    })
    ...
    test('minLength rule validates correctly', () => {
      ...
      expect(minLength3('hi', 'Password')).toBe('Password muss mindestens 3 Zeichen lang sein')
  ...
  describe('createValidator', () => {
    test('creates composite validator', () => {
      ...
      const errors1 = validator({ name: '', email: 'valid@example.com', password: 'secret123' })
      expect(errors1.name).toBe('name ist erforderlich')
      expect(errors1.email).toBe(undefined)
      expect(errors1.password).toBe(undefined)

      const errors2 = validator({ name: 'John', email: 'invalid-email', password: 'short' })
      expect(errors2.name).toBe(undefined)
      expect(errors2.email).toBe('Bitte geben Sie eine gültige E-Mail-Adresse ein')
      expect(errors2.password).toBe('password muss mindestens 6 Zeichen lang sein')

      const errors3 = validator({ name: 'John', email: 'valid@example.com', password: 'secret123' })
  ...
  describe('formPersistence', () => {
    test('saves and loads form state', () => {
      ...
      expect(mockLocalStorage.setItem).toHaveBeenCalledWith('form-test-form', JSON.stringify(testData))
    ...
    test('works when window is undefined', () => {
      ...
      expect(() => formPersistence.save('test-form', {})).not.toThrow()
      expect(formPersistence.load('test-form', { default: true })).toEqual({ default: true })
    })
  })
  ...
  describe('createFormSubmitHandler', () => {
    ...
    test('handles submit errors', async () => {
      ...
      const mockSubmitFn = jest.fn().mockRejectedValue(new Error('Network error'))
