...
describe('Design Tokens Stability', () => {
  ...
  test('tokens.json structure is valid', async () => {
    ...
    const essentialColors = ['color-primary', 'color-accent', 'color-bg', 'color-surface', 'color-text'];
  ...
  test('tokens.json matches CSS variables', async () => {
    ...
    expect(cssContent).toContain(`--color-primary: ${tokens.light.colors['color-primary']}`);
    expect(cssContent).toContain(`--color-accent: ${tokens.light.colors['color-accent']}`);
    ...
    // Check spacing
    expect(cssContent).toContain(`--space-1: ${tokens.light.spacing['space-1']}`);
    expect(cssContent).toContain(`--space-4: ${tokens.light.spacing['space-4']}`);
  });
});