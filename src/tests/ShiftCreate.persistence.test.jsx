...
describe('Shift create persistence', () => {
  test('marks shift pendingSync when repository create fails', () => {
    const failingRepo = { list: () => Promise.resolve([]), create: () => Promise.reject(new Error('fail')), ping: () => Promise.resolve(true) }
    render(
      <ShiftProvider disableAsyncBootstrap repositoryOverride={failingRepo} enableAsyncInTests>
        <CreateButton shift={{ date: new Date('2025-08-26'), type: 'Test', start: '08:00', end: '12:00' }} />
      </ShiftProvider>
    )
