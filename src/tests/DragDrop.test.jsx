...
const mockAuth = {
  user: { id: "test-user", name: "Test User", role: "chief" },
  login: jest.fn(),
  logout: jest.fn(),
};
...
describe("Drag & Drop Functionality", () => {
  beforeEach(() => {
    // Minimal localStorage mock (not used for shifts now that we inject directly)
  ...
  test("renders shifts as draggable in week view for managers", async () => {
    ...
    await waitFor(() => {
      const shiftElements = screen.getAllByText(/Morning|Afternoon|Früh|Spät/);
  ...
  test("drag preview shows when dragging shift", async () => {
    ...
    await waitFor(() => {
      const shiftElement = screen.getAllByText(
        /Morning|Afternoon|Früh|Spät/,
      )[0];
  ...
  test("maintains shift data integrity after drag operations", async () => {
    ...
    await waitFor(() => {
      const els = screen.getAllByText(/Morning|Afternoon|Früh|Spät/);
