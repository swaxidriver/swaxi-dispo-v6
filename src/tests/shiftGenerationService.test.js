...
describe("shiftGenerationService.generateShifts", () => {
  ...
  it("handles cross-midnight templates correctly", () => {
    ...
    expect(nightShift.end_dt.utc.getTime()).toBeGreaterThan(
      nightShift.start_dt.utc.getTime(),
    );
  });
  ...
  it("validates templates and filters invalid ones", () => {
    const templates = [
      ...
      {
        name: "cross-midnight-invalid",
        startTime: "22:00",
        endTime: "06:00",
        days: ["Mo"],
      }, // Missing cross_midnight flag
    ];
    ...
    expect(
      shifts.filter((s) => s.name === "cross-midnight-invalid").length,
    ).toBe(0);
...
describe("createSwaxiDefaultTemplates", () => {
  it("creates valid Swaxi operating pattern templates", () => {
    ...
    const weekdayEvening = templates.find(
      (t) => t.name === "Abend" && t.days.includes("Mo"),
    );
    ...
    const weekdayNight = templates.find(
      (t) => t.name === "Nacht" && t.days.includes("Mo"),
    );
    ...
    const weekendEarly = templates.find(
      (t) => t.name === "Früh" && t.days.includes("Fr"),
    );
    ...
    const weekendNight = templates.find(
      (t) => t.name === "Nacht" && t.days.includes("Fr"),
    );
