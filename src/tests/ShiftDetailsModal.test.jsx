...
describe("ShiftDetailsModal", () => {
  ...
  test("calls onApply when apply button is clicked and confirmed", async () => {
    const onApply = jest.fn().mockResolvedValue();

    render(<ShiftDetailsModal {...defaultProps} onApply={onApply} />);

    // Click apply button - should open confirmation dialog
    ...
    // Confirm the application - get all buttons and click the one in the confirmation modal
    const bewerbenButtons = screen.getAllByText("Bewerben");
    expect(bewerbenButtons).toHaveLength(2); // One in shift modal, one in confirmation

    // Click the confirmation button (should be the first one since it's rendered later)
    fireEvent.click(bewerbenButtons[0]);
  ...
  test("shows loading state when applying", async () => {
    ...
    expect(screen.getAllByText("Bewerbe...")).toHaveLength(2); // One in confirmation, one in main modal
  });
  ...
  test("renders nothing when shift is null", () => {
    const { container } = render(
      <ShiftDetailsModal {...defaultProps} shift={null} />,
    );
