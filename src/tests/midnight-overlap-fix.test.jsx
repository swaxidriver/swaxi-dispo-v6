...
describe("P0 Issue: Midnight overlap correctness", () => {
  ...
  test("Over-midnight shift creation and storage", async () => {
    ...
    const updatedNightShift = holder.current.shifts.find(
      (s) => s.type === "Nacht",
    );
  ...
  test("Over-midnight date boundaries are handled correctly", () => {
    ...
    expect(overlaps(shiftA.start, shiftA.end, shiftB.start, shiftB.end)).toBe(
      false,
    );
    ...
    expect(
      overlaps(
        shiftA.start,
        shiftA.end,
        shiftB_sameDate.start,
        shiftB_sameDate.end,
      ),
    ).toBe(false); // Exact boundary
  });
