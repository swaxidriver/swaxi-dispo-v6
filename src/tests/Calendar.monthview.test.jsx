...
describe('Calendar Month/Week View', () => {
  ...
  it('renders with week view by default', () => {
    ...
    expect(screen.getByRole('button', { name: 'Woche' })).toHaveClass('bg-[var(--color-primary)]')
    expect(screen.getByRole('button', { name: 'Monat' })).not.toHaveClass('bg-[var(--color-primary)]')
  ...
  it('switches to month view when Monat button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Monatsübersicht der Dienste')).toBeInTheDocument()
    })
    
    expect(monthButton).toHaveClass('bg-[var(--color-primary)]')
    expect(screen.getByRole('button', { name: 'Woche' })).not.toHaveClass('bg-[var(--color-primary)]')
  ...
  it('shows appropriate navigation buttons for week view', () => {
    ...
    expect(screen.getByRole('button', { name: 'Vorherige Woche' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Nächste Woche' })).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Vorheriger Monat' })).not.toBeInTheDocument()
  ...
  it('shows appropriate navigation buttons for month view', async () => {
    ...
    expect(screen.queryByRole('button', { name: 'Vorherige Woche' })).not.toBeInTheDocument()
