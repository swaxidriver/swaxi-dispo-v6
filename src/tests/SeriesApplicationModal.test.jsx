...
describe("SeriesApplicationModal", () => {
  ...
  it("renders available shifts and allows selecting multiple via type shortcut", () => {
    ...
    // Submit - this should now open confirmation dialog
    ...
    expect(bewerbenButtons).toHaveLength(2); // One in series modal, one in confirmation

    // Click the confirmation button (should be the second one)
    fireEvent.click(bewerbenButtons[1]);

    expect(mockApplyToSeries).toHaveBeenCalledTimes(1);
    const [ids, user, comment] = mockApplyToSeries.mock.calls[0];
    expect(ids.sort()).toEqual(["s1", "s2"]);
    expect(user).toBe("current-user");
    expect(comment).toBe(""); // No comment provided
  });
