...
describe('ConflictBadge', () => {
  ...
  it('renders conflict badge with single conflict', () => {
    ...
    expect(screen.getByLabelText(/1 Konflikt.*Zeitüberlappung/)).toBeInTheDocument()
  })

  it('renders conflict badge with multiple conflicts', () => {
    render(<ConflictBadge conflicts={['TIME_OVERLAP', 'DOUBLE_APPLICATION']} />)
    
    expect(screen.getByText('2 Konflikte')).toBeInTheDocument()
    expect(screen.getByLabelText(/2 Konflikte.*Zeitüberlappung.*Doppelte Bewerbung/)).toBeInTheDocument()
  })
  ...
  it('shows tooltip with conflict descriptions on hover', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Zeitüberlappung, Doppelte Bewerbung')).toBeInTheDocument()
    })
  })
  ...
  it('handles unknown conflict codes gracefully', () => {
    ...
    expect(screen.getByText('1 Konflikt')).toBeInTheDocument()
    expect(screen.getByLabelText(/1 Konflikt.*UNKNOWN_CONFLICT/)).toBeInTheDocument()
  })

  it('applies custom className', () => {
    render(<ConflictBadge conflicts={['TIME_OVERLAP']} className="custom-class" />)
  ...
  it('has proper styling classes', () => {
    ...
    const badge = screen.getByText('1 Konflikt').parentElement
    expect(badge).toHaveClass('inline-flex', 'items-center', 'px-2', 'py-1', 'text-xs', 'font-medium', 'text-red-800', 'bg-red-100', 'border', 'border-red-200', 'rounded-full')
  })
})