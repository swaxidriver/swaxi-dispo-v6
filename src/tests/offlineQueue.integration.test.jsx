...
function TestComp({ createArgs }) {
  ...
  React.useEffect(() => {
    if (createArgs) createShift(createArgs);
  }, [createArgs, createShift]);
  return (
    <div data-testid="meta">
      {JSON.stringify({
        count: shifts.length,
        isOnline: state.isOnline,
        ids: shifts.map((s) => ({ id: s.id, pendingSync: s.pendingSync })),
      })}
    </div>
  );
}
...
describe("offline queue integration", () => {
  ...
  test("failed create is queued and pendingSync then drained when online", async () => {
    ...
    const succeedingRepo = {
      list: () => [],
      create: (s) => Promise.resolve({ ...s, extra: "ok" }),
      ping: () => Promise.resolve(true),
    };

    render(
      <ShiftProvider
        repositoryOverride={succeedingRepo}
        disableAsyncBootstrap
        enableAsyncInTests
      >
        <TestComp />
    ...
    await act(async () => {
      succeedingRepo.ping = () => Promise.resolve(true);
    });
  ...
  test("apply action enqueued on failure and retried", async () => {
    const failingRepo = {
      ...
      applyToShift: () => Promise.reject(new Error("offline")),
      ping: () => Promise.resolve(false),
    };
    ...
    function ApplyComp() {
      ...
      React.useEffect(() => {
        ...
        setTimeout(
          () => applyToShift(`${d.toISOString().slice(0, 10)}_Y`, "user1"),
          0,
        );
