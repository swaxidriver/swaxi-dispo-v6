...
import {
  ...
  CONFLICT_CODES,
} from "../features/shifts/shifts.js";
...
describe("Rule Engine Integration - Double-booking Prevention", () => {
  ...
  describe("Case 1: Rule blocks overlap without override", () => {
    test("should prevent assignment due to time overlap and assignment collision", () => {
      ...
      const enforcement = ruleEngine.enforceRules(
        newShift,
        [existingShift],
        [],
        testActor,
      ...
      expect(enforcement.message).toContain(
        "Assignment blocked by rules: Prevent Double Booking",
      );
    });
  });
  ...
  describe("Case 2: Exception with note allows assignment", () => {
    test("should allow assignment with valid override and reason", () => {
      ...
      const initialEvaluation = ruleEngine.evaluateRules(
        newShift,
        [existingShift],
        [],
      );
      ...
      const overrideResult = ruleEngine.createOverride(
        ...
        {
          reason:
            "Critical emergency coverage required due to staff illness. Approved by operations manager.",
      ...
      expect(overrideResult.override.reason).toContain(
        "Critical emergency coverage",
      );
      expect(overrideResult.override.approver).toBe("Operations Manager");

      // Step 3: Re-evaluate with override in place
      const overriddenEvaluation = ruleEngine.evaluateRules(
        newShift,
        [existingShift],
        [],
      );
      ...
      expect(enforcement.message).toBe(
  ...
  describe("Complex Scenarios", () => {
    test("should handle multiple overlapping shifts with mixed violations", () => {
      ...
      const doubleBookingViolation = evaluation.violations.find(
        (v) => v.rule.id === "PREVENT_DOUBLE_BOOKING",
      );
      ...
      const locationViolation = evaluation.violations.find(
    ...
    test("should handle short turnaround scenarios across days", () => {
      ...
      const restViolation = evaluation.violations.find(
  ...
  describe("Override Edge Cases", () => {
    ...
    test("should properly remove and audit override removal", () => {
      ...
      const removeResult = ruleEngine.removeOverride(
