...
import { ASSIGNMENT_STATUS, dayNamesToMask, maskToDayNames, isDayActive } from '../../repository/schemas';
...
describe('Data Model Implementation', () => {
  ...
  describe('ShiftTemplate Operations', () => {
    ...
    test('should list templates with filters', async () => {
      ...
      const activeTemplates = await repository.listShiftTemplates({ active: true });
  ...
  describe('ShiftInstance Operations', () => {
    ...
    test('should filter instances by date and template', async () => {
      ...
      const instancesByDate = await repository.listShiftInstances({ date: '2025-01-15' });
      const instancesByTemplate = await repository.listShiftInstances({ template_id: template.id });
  ...
  describe('Weekday Mask Utilities', () => {
    test('should convert day names to mask correctly', () => {
      ...
      const allDays = dayNamesToMask(['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su']);
