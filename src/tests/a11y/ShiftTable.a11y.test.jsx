...
describe("ShiftTable Accessibility", () => {
  ...
  test("has no axe violations with large virtualized dataset", async () => {
    const shifts = Array.from({ length: 150 }, (_, i) =>
      generateTestShift(i + 1),
    );
  ...
  test("keyboard navigation works correctly", async () => {
    const shifts = Array.from({ length: 120 }, (_, i) =>
      generateTestShift(i + 1),
    ); // Use virtualization
  ...
  test("series application button has proper accessibility attributes", () => {
    ...
    expect(seriesButton.getAttribute("aria-label")).toContain(
      "Serienbewerbung für 3 offene Dienste",
    );
  });

  test("virtualized list maintains accessibility with large datasets", () => {
    const shifts = Array.from({ length: 200 }, (_, i) =>
      generateTestShift(i + 1),
    );
