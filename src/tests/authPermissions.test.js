import { canManageShifts, canViewAudit, canApplyForShifts, canViewAnalytics } from '../utils/auth'
...
describe('auth permission helpers', () => {
  ...
  it('role hierarchy enforces expected permissions', () => {
    ...
    expect(admin).toMatchObject({ manage: true, audit: true, apply: true, analytics: true })

    const chief = matrix.find(m => m.role === ROLES.CHIEF)
    expect(chief).toMatchObject({ manage: true, audit: false, apply: true, analytics: true })

    const disponent = matrix.find(m => m.role === ROLES.DISPONENT)
    expect(disponent).toMatchObject({ manage: false, audit: false, apply: true, analytics: true })

    const analyst = matrix.find(m => m.role === ROLES.ANALYST)
    expect(analyst).toMatchObject({ manage: false, audit: false, apply: false, analytics: true })
  })
