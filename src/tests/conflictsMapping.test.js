...
describe("conflicts mapping utilities", () => {
  test("CONFLICT_LABELS keys stable", () => {
    expect(Object.keys(CONFLICT_LABELS).sort()).toEqual(
      [
  ...
  test("getConflictSeverity returns correct severity levels", () => {
    expect(getConflictSeverity("TIME_OVERLAP")).toBe(
      CONFLICT_SEVERITY.BLOCKING,
    );
    expect(getConflictSeverity("ASSIGNMENT_COLLISION")).toBe(
      CONFLICT_SEVERITY.BLOCKING,
    );
    expect(getConflictSeverity("SHORT_TURNAROUND")).toBe(
      CONFLICT_SEVERITY.WARNING,
    );
    expect(getConflictSeverity("DOUBLE_APPLICATION")).toBe(
      CONFLICT_SEVERITY.WARNING,
    );
    expect(getConflictSeverity("LOCATION_MISMATCH")).toBe(
      CONFLICT_SEVERITY.WARNING,
    );
