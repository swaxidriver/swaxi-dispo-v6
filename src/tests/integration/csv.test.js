...
class MockRepository {
  ...
  async updatePerson(id, updates) {
    ...
    if (index >= 0) {
      this.people[index] = { ...this.people[index], ...updates, updated_at: new Date() }
  ...
  async updateShiftTemplate(id, updates) {
    ...
    if (index >= 0) {
      this.shiftTemplates[index] = { ...this.shiftTemplates[index], ...updates, updated_at: new Date() }
...
describe('CSV Parser', () => {
  ...
  describe('arrayToCSV', () => {
    ...
    test('should escape fields with commas and quotes', () => {
      const data = [
        { name: 'Doe, John', quote: 'He said "Hello"' }
      ]
      const result = arrayToCSV(data)
...
describe('Shift Templates Import', () => {
  ...
  test('should import new shift templates successfully', async () => {
    const csv = 'name,start_time,end_time,weekday_mask,cross_midnight\nMorning,08:00,16:00,31,false\nNight,22:00,06:00,127,true'
  ...
  test('should parse days field instead of weekday_mask', async () => {
    const csv = 'name,start_time,end_time,days\nWeekday,09:00,17:00,"Mo,Tu,We,Th,Fr"'
  ...
  test('should require either weekday_mask or days', async () => {
    ...
    expect(result.created).toBe(0)
    expect(result.errors).toHaveLength(1)
    expect(result.errors[0]).toContain('Either weekday_mask or days field is required')
  })
})
...
describe('Export Functions', () => {
  ...
  describe('exportShiftTemplates', () => {
    test('should export shift templates to CSV format', async () => {
      ...
      expect(result).toContain('name,weekday_mask,days,start_time,end_time,cross_midnight')
      expect(result).toContain('Morning,31,"Mo,Tu,We,Th,Fr",08:00,16:00,false')
      expect(result).toContain('Night,127,"Mo,Tu,We,Th,Fr,Sa,Su",22:00,06:00,true')
    })
  })
  ...
  describe('exportAssignments', () => {
    test('should export assignments with cross-midnight and paid hours', async () => {
      ...
      expect(result).toContain('date,shift_name,start_time,end_time,cross_midnight,duration_minutes,paid_hours')
  ...
  describe('exportPerdisWebComm', () => {
    test('should export in Perdis/WebComm format', async () => {
      ...
      expect(result).toContain('PersonalNr,Name,Datum,Schicht,Von,Bis,Stunden,Mitternacht,Status')
      expect(result).toContain('person-1,John Doe,2025-01-06,Morning,08:00,16:00,8.00,0,assigned')
      expect(result).toContain('person-2,Jane Smith,2025-01-06,Night,22:00,06:00,8.00,1,assigned')
    })
...
describe('Error Handling', () => {
  test('should handle repository errors gracefully', async () => {
    ...
    await expect(exportPeople(failingRepository))
      .rejects.toThrow('Failed to export people: Database error')
  })
