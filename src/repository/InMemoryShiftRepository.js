...
export class InMemoryShiftRepository extends ShiftRepository {
  ...
  async list() { return this.shifts }
  async create(shift) {
    const withMeta = { ...shift, id: shift.id || Date.now(), createdAt: new Date(), updatedAt: new Date() }
    this.shifts.push(withMeta)
    return withMeta
  }
  async update(id, patch) {
    this.shifts = this.shifts.map(s => s.id === id ? { ...s, ...patch, updatedAt: new Date() } : s)
