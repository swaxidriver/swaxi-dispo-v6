...
export default function MiniAnalytics({ onViewSource }) {
  ...
  const stats = useMemo(() => {
    ...
    const applicationsLast7d = state.applications.filter(
      (app) => app.ts && app.ts >= sevenDaysAgo.getTime(),
    ).length;
  ...
  const getIcon = (iconType) => {
    ...
    switch (iconType) {
      case "clock":
        return (
          <div
            className={`${iconClass} rounded-full border-2 border-current`}
          />
        );
      case "check":
        return (
      ...
      case "warning":
        return (
          <div className={`${iconClass} relative`}>
            <div
              className="absolute inset-1 border-2 border-current border-b-0 transform rotate-180"
      ...
      case "user":
        return (
          <div
            className={`${iconClass} rounded-full border-2 border-current relative`}
          >
