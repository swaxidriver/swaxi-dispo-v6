...
export default function ShiftTable({ shifts, showActions = true }) {
  ...
  return (
    <div className="bg-white shadow overflow-hidden sm:rounded-md">
            ...
            <div className="px-4 py-4 sm:px-6">
              <div className="flex items-center justify-between">
                <div className="text-sm font-medium text-[var(--color-primary)] truncate">
                  ...
                  <span className="ml-2 text-xs text-gray-500">
                    ({(computeDuration(shift.start, shift.end) / 60).toFixed(1)}
                    h)
                  </span>
                </div>
                <div className="ml-2 flex-shrink-0 flex">
                  <span
                    className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusBadgeClass(shift.status)}`}
                  >
                    {shift.status}
                  </span>
                        ...
                        {(() => {
                          ...
                          return (
                            <button
                              ...
                              className={`inline-flex items-center rounded-md px-3 py-2 text-sm font-semibold shadow-sm ${applyDisabled ? "bg-gray-300 text-gray-600 cursor-not-allowed" : "btn-primary"}`}
                              title={applyReason}
                          ...
                          (() => {
                            ...
                            return (
                              <button
                                ...
                                className={`inline-flex items-center rounded-md px-3 py-2 text-sm font-semibold shadow-sm ring-1 ring-inset ${assignDisabled ? "bg-gray-200 text-gray-500 cursor-not-allowed ring-gray-200" : "bg-white text-gray-900 ring-gray-300 hover:bg-gray-50"}`}
                                title={assignReason}
                      ...
                      (() => {
                        ...
                        return (
                          <button
                            ...
                            className={`inline-flex items-center rounded-md px-3 py-2 text-sm font-semibold shadow-sm ${cancelDisabled ? "bg-gray-300 text-gray-600 cursor-not-allowed" : "bg-red-600 text-white hover:bg-red-500"}`}
                            title={cancelReason}
              ...
              {shift.conflicts?.length > 0 && (
                ...
                >
                  ...
                  {describeConflicts(shift.conflicts).join(", ")}
                </div>
              )}
            </div>
          </li>
      ...
      {showActions &&
        shifts.filter((s) => s.status === SHIFT_STATUS.OPEN).length > 1 && (
