...
function ShiftTable({ shifts, showActions = true }) {
  ...
  const [assignmentModalShiftId, setAssignmentModalShiftId] = useState(null);
  const auth = useContext(AuthContext);
  const userRole = auth?.user?.role || "analyst";
  const isMobileDevice = useMobileDevice();

  // Use mobile device detection with window width fallback for better reliability
  const isMobile =
    isMobileDevice ||
  ...
  const handleAssign = useCallback(
    (shiftId) => {
      if (!auth?.user) return;
      // Open the assignment modal instead of direct assignment
      setAssignmentModalShiftId(shiftId);
    },
    [auth?.user],
  );
        ...
        >
          <div className="flex items-center justify-between">
            <div className="text-sm font-medium text-[var(--color-primary)] truncate">
              ...
              >
                ({(computeDuration(shift.start, shift.end) / 60).toFixed(1)}h)
              </span>
            </div>
            <div
          ...
          >
            ...
            {showActions && (
              <div
                className="flex items-center text-sm text-gray-500 sm:mt-0"
                    ...
                    {(() => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            !applyDisabled && handleApply(shift.id)
                          }
                          className={`inline-flex items-center rounded-md text-sm font-semibold shadow-sm ${applyDisabled ? "bg-gray-300 text-gray-600 cursor-not-allowed" : "btn-primary"}`}
                          title={applyReason}
                      ...
                      (() => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() =>
                              !assignDisabled && handleAssign(shift.id)
                            }
                            data-testid="assign-shift-btn"
                            className={`inline-flex items-center rounded-md text-sm font-semibold shadow-sm ring-1 ring-inset ${assignDisabled ? "bg-gray-200 text-gray-500 cursor-not-allowed ring-gray-200" : "bg-white text-gray-900 ring-gray-300 hover:bg-gray-50"}`}
                            title={assignReason}
                  ...
                  (() => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() =>
                          !cancelDisabled && handleCancel(shift.id)
                        }
                        className={`inline-flex items-center rounded-md text-sm font-semibold shadow-sm ${cancelDisabled ? "bg-gray-300 text-gray-600 cursor-not-allowed" : "bg-red-600 text-white hover:bg-red-500"}`}
                        title={cancelReason}
  ...
  const renderVirtualizedShift = useCallback(
    (shift) => {
      // Use mobile card layout for virtualized items on mobile
      if (isMobile) {
        return (
          <ShiftCard
            key={shift.id}
      ...
      // Desktop list layout for virtualized items
      return (
        <div
          key={shift.id}
          style={{
  ...
  return (
    <div
      className={`bg-white shadow overflow-hidden sm:rounded-md ${isMobile ? "mobile-shift-container" : ""}`}
    ...
    >
      {shouldVirtualize ? (
        ...
        <VirtualizedList
          items={shifts}
          itemHeight={isMobile ? 200 : 120} // Taller items on mobile for cards
      ...
      isMobile ? (
        // Mobile card layout
        <div
          className="mobile-shift-cards"
          role="list"
          data-testid="shift-cards"
          style={{
            padding: "var(--space-4)",
          }}
        >
      ...
      <div
        className="bg-white border-t text-xs text-gray-500 space-y-1"
        style={{
          paddingLeft: "var(--space-4)",
          paddingRight: "var(--space-4)",
          paddingTop: "var(--space-2)",
      ...
      >
        <div className="flex flex-wrap" style={{ gap: "var(--space-3)" }}>
          ...
          <span>
            <span className="font-semibold">Zeitüberlappung</span> =
            Überschneidung in Zeit
          </span>
          <span>
            <span className="font-semibold">Doppelte Bewerbung</span> = Bewerber
            in overlappenden Diensten
          </span>
          <span>
            <span className="font-semibold">Zuweisungs-Kollision</span> = Person
            doppelt zugewiesen
          </span>
          <span>
            <span className="font-semibold">Standort-Konflikt</span> =
            Unterschiedliche Orte gleichzeitig
          </span>
        </div>
      </div>
      ...
      <SingleAssignmentModal
        isOpen={assignmentModalShiftId !== null}
        onClose={() => setAssignmentModalShiftId(null)}
        shiftId={assignmentModalShiftId}
