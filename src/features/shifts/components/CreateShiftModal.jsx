...
export default function CreateShiftModal({ isOpen, onClose, defaultDate }) {
    ...
    async (shiftData) => {
      ...
      if (!res.ok) {
        if (res.reason === "duplicate") {
          throw new Error(
            t("shiftExistsAlready") || "Dienst existiert bereits",
          );
        } else if (res.reason === "workLocation") {
          throw new Error(
            t("workLocationRequired") || "Arbeitsort erforderlich",
          );
        } else {
          throw new Error(
            t("errorCreatingShift") || "Fehler beim Erstellen des Dienstes",
          );
        }
      }
      return res;
  ...
  const handleSubmit = createFormSubmitHandler(
    ...
    createShiftOp.execute,
    {
      ...
      successMessage:
        t("shiftCreatedSuccessfully") || "Dienst erfolgreich erstellt!",
      clearOnSuccess: true,
      onSuccess: () => {
        localStorage.removeItem("swaxi-unsaved-work");
        onClose();
      ...
      >
        ...
        <div className="space-y-1">
          ...
          <select
            ...
            aria-describedby={
              getFieldError("workLocation") ? "workLocation-error" : undefined
            }
          >
            <option value="">-- bitte wählen --</option>
