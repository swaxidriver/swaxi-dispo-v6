...
export default function SeriesApplicationModal({
  ...
  const conflictData = useMemo(() => {
    ...
    const selectedShiftObjects = selectedShifts
      .map((id) => availableShifts.find((s) => s.id === id))
      .filter(Boolean);
  ...
  const handleConfirmApply = async (comment) => {
    applyToSeries(selectedShifts, userId, comment);
    onClose();
    setSelectedShifts([]);
  };
  ...
  return (
    <>
      ...
      >
        <div
          className="relative mx-auto border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white"
          style={{ top: "var(--space-16)", padding: "var(--space-4)" }}
          role="document"
        >
          <div style={{ marginTop: "var(--space-3)" }}>
            ...
            <div style={{ marginBottom: "var(--space-4)" }}>
              <p
                id="series-modal-desc"
                className="text-sm text-gray-600"
                style={{ marginBottom: "var(--space-3)" }}
              >
                Bewerben Sie sich für mehrere Dienste gleichzeitig. Wählen Sie
                die gewünschten Dienste aus oder nutzen Sie die Schnellauswahl.
              </p>
            ...
            {conflictData.hasConflicts && (
              <div
                className="border border-red-200 bg-red-50 rounded-md"
                style={{
                  padding: "var(--space-3)",
              ...
              >
                ...
                <ul className="space-y-1">
                  {describeConflicts(conflictData.allConflictReasons).map(
                    (desc, index) => (
                      <li
                        key={index}
            ...
            >
              ...
              <div className="flex" style={{ gap: "var(--space-3)" }}>
                ...
                <button
                  ...
                  disabled={
                    selectedShifts.length === 0 || conflictData.hasConflicts
                  }
