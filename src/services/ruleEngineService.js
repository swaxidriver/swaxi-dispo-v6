...
export class RuleEngineService {
  ...
  static async validateAssignment(
    ...
    try {
      ...
      return {
        ...
        requiresOverride:
          !evaluation.canAssign &&
          evaluation.violations.some((v) => v.canOverride),
      };
    } catch (error) {
  ...
  static async checkAssignmentPermission(
    ...
    const hasOverridableViolations = validation.violations.some(
      (v) => v.canOverride,
    );
  ...
  static async createOverride(shift, ruleId, overrideData) {
    try {
      ...
      const result = ruleEngine.createOverride(
        shift,
        ruleId,
        overrideData,
        actor,
   ...
   */
  static async enforceAssignment(
    shift,
    existingShifts = [],
    applications = [],
    options = {},
    ...
    try {
      ...
      const result = ruleEngine.enforceRules(
        shift,
        existingShifts,
        applications,
        actor,
  ...
  static getActiveOverrides() {
    try {
      return ruleEngine.getActiveOverrides().map((override) => ({
        ...
        ruleName:
          ruleEngine.getRules()[override.ruleId]?.name || override.ruleId,
      }));
    } catch (error) {
