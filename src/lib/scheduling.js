...
export function calculateTimeOverlap(aStart, aEnd, bStart, bEnd) {
  ...
  const toMinutes = (timeStr) => {
    if (typeof timeStr !== "string" || !/^[0-2]\d:[0-5]\d$/.test(timeStr))
      return NaN;
...
export function findSchedulingConflicts(
  ...
  if (overlappingShifts.length > 0) {
    ...
    if (targetShift.assignedTo) {
      const hasAssignmentCollision = overlappingShifts.some(
        (shift) => shift.assignedTo === targetShift.assignedTo,
      );
  ...
  if (applications.length > 0) {
    const targetApplications = applications.filter(
      (app) => app.shiftId === targetShift.id,
    );
    ...
    const hasDoubleApplication = applications.some(
      (app) =>
...
export function checkShortTurnaround(shiftA, shiftB, minRestMinutes = 480) {
  ...
  if (!shiftA.assignedTo || shiftA.assignedTo !== shiftB.assignedTo)
    return false;
