...
const spacingRegex =
  /(?:(?:padding|margin|gap)(?:-(?:top|left|right|bottom|x|y))?:\s*(\d+)px|[pm][xytlrb]?-(\d+)|space-[xy]?-(\d+))/g;

async function run() {
  const entries = await fg(["**/*.{js,jsx,ts,tsx,css,scss}"], {
  ...
  for (const file of entries) {
    ...
    while ((match = spacingRegex.exec(content)) !== null) {
      ...
      if (
        pixelValue &&
        !allowedSpacingValues.has(pixelValue) &&
        !allowedFractionalValues.has(pixelValue)
      ) {
        ...
        if (
          !/(?:height|width|size|weight|line-height|font|border|shadow|z-index|duration|delay|radius|transform|top|left|right|bottom)(?![a-z])/i.test(
            context,
          )
        ) {
          matches.push({
  ...
  if (violations.length) {
    ...
    console.error(
      "Standard scale: 4, 8, 12, 16, 24, 32, 64px (--space-1 through --space-16)\n",
    );
