...
const jestCmd = ['npx','jest','--ci','--json',`--outputFile=${jestJson}`,'--coverage','--coverageReporters=json-summary','--reporters=default','--reporters=summary']
...
let coverageSummary = {}
const covSummaryPath = path.join(projectRoot, 'coverage', 'coverage-summary.json')
if (existsSync(covSummaryPath)) {
  try { coverageSummary = JSON.parse(readFileSync(covSummaryPath,'utf8')) } catch { /* ignore */ }
}

const globalCov = coverageSummary.total || {}
function pct(v) { return typeof v?.pct === 'number' ? v.pct : null }
const nearThreshold = []
for (const key of ['statements','branches','functions','lines']) {
  const val = pct(globalCov[key])
  if (val != null && thresholds[key] != null) {
    const diff = val - thresholds[key]
    if (diff >= 0 && diff <= 2) nearThreshold.push({ metric: key, value: val, threshold: thresholds[key], delta: +diff.toFixed(2) })
    if (diff < 0) nearThreshold.push({ metric: key, value: val, threshold: thresholds[key], delta: diff, below: true })
  }
}
...
if (failingTests.length) {
  for (const f of failingTests) actionItems.push({ priority: 1, type: 'test-failure', test: f.fullName, file: f.file })
}

const sortedLintRules = Object.entries(lintErrorByRule).sort((a,b)=>b[1]-a[1])
for (const [rule, count] of sortedLintRules) actionItems.push({ priority: 2, type: 'lint-rule', rule, count })

for (const cov of nearThreshold.filter(c=>c.below)) actionItems.push({ priority: 3, type: 'coverage-below', ...cov })
for (const cov of nearThreshold.filter(c=>!c.below && c.delta <= 2)) actionItems.push({ priority: 4, type: 'coverage-near', ...cov })

const summary = {
  stats: {
    tests: { total: jestData.numTotalTests, failed: jestData.numFailedTests, passed: jestData.numPassedTests },
    suites: { total: jestData.numTotalTestSuites, failed: jestData.numFailedTestSuites },
    lint: { errors: lintErrors.length, filesWithErrors: new Set(lintErrors.map(e=>e.filePath)).size },
    coverage: Object.fromEntries(['statements','branches','functions','lines'].map(k=>[k, pct(globalCov[k])]))
  },
  thresholds,
  actionItems,
}

writeFileSync(path.join(reportsDir,'quality-summary.json'), JSON.stringify(summary,null,2))

// Human readable output
function formatAction(a){
  switch(a.type){
    case 'test-failure': return `❌ Test failed: ${a.test} (${a.file})`
    case 'lint-rule': return `⚠️  Lint rule ${a.rule} (${a.count} errors)`
    case 'coverage-below': return `🔴 Coverage BELOW threshold: ${a.metric} ${a.value}% < ${a.threshold}% (Δ ${a.delta})`
    case 'coverage-near': return `🟡 Coverage near threshold: ${a.metric} ${a.value}% (threshold ${a.threshold}%, buffer ${a.delta})`
    default: return JSON.stringify(a)
  }
}

console.log('\n=== QUALITY SUMMARY ===')
console.log(`Tests: ${summary.stats.tests.passed}/${summary.stats.tests.total} passed | Suites failed: ${summary.stats.suites.failed}`)
console.log(`Lint errors: ${summary.stats.lint.errors}`)
console.log('Coverage:', Object.entries(summary.stats.coverage).map(([k,v])=>`${k}:${v}%`).join(' '))
