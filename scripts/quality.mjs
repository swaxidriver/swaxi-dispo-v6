...
const lintErrorByRule = lintErrors.reduce((acc, e) => {
  acc[e.ruleId || "unknown"] = (acc[e.ruleId || "unknown"] || 0) + 1;
  return acc;
}, {});
...
const failingTests = (jestData.testResults || []).flatMap((tr) =>
  tr.assertionResults
    .filter((a) => a.status === "failed")
    .map((a) => ({ fullName: a.fullName, file: tr.name })),
);
...
try {
  ...
  const match = jestConfigSource.match(
    /coverageThreshold:[\s\S]*?global:[\s\S]*?{([\s\S]*?)}[\s\S]*?}/,
  );
  if (match) {
