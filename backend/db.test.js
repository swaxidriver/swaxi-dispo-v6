...
describe("Database Integration Tests - Constraints & Transactions", () => {
  ...
  describe("Swap Transactions", () => {
    ...
    test("should rollback failed swap transaction", async () => {
      ...
      await expect(
        repository.swapAssignments(assignment1.id, "non-existent-id"),
      ).rejects.toThrow();

      // Verify original assignments are unchanged
      const unchangedAssignment1 = await repository.getAssignment(
        assignment1.id,
      );
      const unchangedAssignment2 = await repository.getAssignment(
        assignment2.id,
      );
  ...
  describe("Soft Delete and Cascade", () => {
    ...
    test("should cascade delete shift instance and related assignments", async () => {
      ...
      const deletedShiftInstance = await repository.getShiftInstance(
        shiftInstance.id,
      );
    ...
    test("should soft delete shift template and deactivate instances", async () => {
      ...
      const instance1Updated = await repository.getShiftInstance(
        shiftInstance.id,
      );
    ...
    test("should handle referential integrity on hard delete attempts", async () => {
      ...
      await expect(repository.hardDeletePerson(person.id)).rejects.toThrow(
        "Cannot delete person with existing assignments",
      );
