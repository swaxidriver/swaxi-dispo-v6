  ...
  if (authHeader) {
    ...
    switch (token) {
      ...
      case "disponent-token":
        req.user = {
          role: ROLES.DISPONENT,
          email: "disp@stadtwerke-augsburg.de",
          name: "Dispatcher",
        };
        break;
      case "analyst-token":
        req.user = {
          role: ROLES.ANALYST,
          email: "analyst@stadtwerke-augsburg.de",
          name: "Analyst",
        };
        break;
    }
...
app.get(
  "/api/management",
  requireRole([ROLES.ADMIN, ROLES.CHIEF]),
  (req, res) => {
    res.json({
...
app.get("/api/templates", guardResource("templates", "read"), (req, res) => {
  res.json({
    ...
    user: getUserContext(req),
    templates: [{ id: 1, name: "Frühdienst", start: "06:00", end: "14:00" }],
  });
});
...
app.post(
  ...
  async (req, res) => {
    try {
      const { type, email, shift } = req.body;
      await notificationService.queueAssignmentNotification({
        shiftId: "test_shift",
        assignedTo: email,
        type: type || "assigned",
        shift: shift || {
...
app.listen(PORT, () => {
  ...
  console.log("   - Daily digest at 18:30");
  console.log("   - Immediate removal notices enabled");
  console.log("");
  ...
  console.log(
    'curl -H "Authorization: Bearer admin-token" http://localhost:3001/api/notifications/digest',
  );
  console.log("");
  console.log("# Chief (can manage but not audit)");
  console.log(
    'curl -H "Authorization: Bearer chief-token" http://localhost:3001/api/shifts -X POST -H "Content-Type: application/json" -d \'{"type":"Früh","date":"2025-01-16"}\'',
  );
  console.log("");
